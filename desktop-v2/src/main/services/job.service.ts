...
export class JobService {
  ...
  async createJob(
    ...
    run(`INSERT INTO jobs (id, type, status, title, total_items, started_at) VALUES (?,?,'running',?,?,datetime('now'))`,
  ...
  private async executeJob(
    ...
    for (const deviceId of deviceIds) {
      ...
      run(`UPDATE job_items SET status='running', started_at=datetime('now') WHERE job_id=? AND device_id=?`, [jobId, deviceId]);
      ...
      try {
        ...
        run(`UPDATE job_items SET status='success', message=?, completed_at=datetime('now') WHERE job_id=? AND device_id=?`,
          [message, jobId, deviceId]);
      } catch (err: any) {
        failed++;
        run(`UPDATE job_items SET status='failed', message=?, completed_at=datetime('now') WHERE job_id=? AND device_id=?`,
    ...
    run(`UPDATE jobs SET status=?, completed_at=datetime('now'), progress=100 WHERE id=?`, [finalStatus, jobId]);
